...
chatRouter.post('/', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const requestHeaders = {
      ...
      // High Effort toggle -> custom_inputs.high_effort (agent vs chat mode)
      ...(highEffort !== undefined
        ? { [CONTEXT_HEADER_HIGH_EFFORT]: String(highEffort) }
        : {}),
