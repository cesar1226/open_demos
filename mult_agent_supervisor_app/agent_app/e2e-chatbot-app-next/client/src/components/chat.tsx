...
import { SparkleDoubleIcon } from '@/components/icons';
...
export function Chat({
  ...
  // "High Effort" toggle: ON -> Genie agent mode (agentic REST), OFF -> chat mode (MCP).
  // Default ON. A ref keeps the value current inside the transport closure.
  const [highEffort, setHighEffort] = useState<boolean>(true);
  const highEffortRef = useRef<boolean>(true);
  highEffortRef.current = highEffort;
      ...
      prepareSendMessagesRequest({ messages, id, body }) {
        ...
        return {
          body: {
            ...
            highEffort: highEffortRef.current,
  ...
  // High Effort toggle + input, used in both the empty and active layouts.
  const inputElement = (
    <div className="flex w-full flex-col gap-1.5">
      <div className="flex justify-end px-1">
        <button
          type="button"
          onClick={() => setHighEffort((v) => !v)}
          aria-pressed={highEffort}
          title="High Effort ON: Genie runs in agent mode (reasons over PDFs). OFF: faster chat mode."
          className={cn(
        ...
        >
          <DbIcon icon={SparkleDoubleIcon} size={14} color={highEffort ? 'ai' : 'muted'} />
          <span>High Effort · {highEffort ? 'On' : 'Off'}</span>
        </button>
      </div>
      {multimodalInput}
    </div>
