...
export const CONTEXT_HEADER_HIGH_EFFORT = 'x-databricks-high-effort';
...
export const databricksFetch: typeof fetch = async (input, init) => {
  ...
  const highEffort = headers.get(CONTEXT_HEADER_HIGH_EFFORT);
  ...
  headers.delete(CONTEXT_HEADER_HIGH_EFFORT);
  ...
  // Inject the High Effort toggle as custom_inputs.high_effort (independent of
  // the conversation/user context above).
  if (
    highEffort !== null &&
    requestInit?.body &&
    typeof requestInit.body === 'string'
