...
This Databricks App routes manual questions across two complementary backends
and keeps conversation history in Lakebase:

| Backend | How it's queried | API used |
|---------|-----------------|----------|
| Topic 1 manuals **Genie space** | AGENT API and Built-in Databricks Genie MCP server |Agent/ MCP for chat |
| Parsed-manual **AI Search index** | Built-in Databricks Vector Search MCP server | MCP |
| Conversation sessions | `AsyncDatabricksSession` | Lakebase |

The orchestrator chooses Genie, AI Search, or both based on the question.

## Configuration

The bundle injects `GENIE_SPACE_ID`, `VECTOR_SEARCH_INDEX`, and
`LAKEBASE_AUTOSCALING_ENDPOINT` from its declared app resources. For local
development, copy `.env.example` to `.env` and provide those values.

## Lakebase conversation memory

The orchestrator uses OpenAI Agents SDK `AsyncDatabricksSession` to persist
short-term conversation history in Lakebase. Requests that carry the same
`custom_inputs.session_id` (or chat `conversation_id`) continue the same
conversation. A new ID is generated when neither is supplied and is returned
as `custom_outputs.session_id` for non-streaming requests.

For deployed apps, the bundle injects `LAKEBASE_AUTOSCALING_ENDPOINT` from the
`lakebase_postgres` app resource. For local development, set that variable in
`.env`. The app service principal also needs schema, table, and sequence
privileges; after deployment, run:

```bash
uv run python scripts/grant_lakebase_permissions.py \
  <app-service-principal-client-id> \
  --memory-type openai \
  --autoscaling-endpoint <projects/.../branches/.../endpoints/...>
```
