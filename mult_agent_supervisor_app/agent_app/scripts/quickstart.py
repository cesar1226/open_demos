...
Steps:
  ...
  4b. Bundle-aware setup (only when NOT binding to an existing app) — locate the
     DAB root databricks.yml (it may be a parent dir) and the resources it pulls
     in via `include:`. If it declares an MLflow experiment and/or a Lakebase
     postgres project, summarize their names, let the user rename them (overwriting
     the yaml where the value lives — a ${var.*} project id edits the variable
     default), then optionally `databricks bundle deploy` (with confirmation,
     default target) so the BUNDLE owns creation, and read the created experiment
     id + Lakebase endpoint back into .env. When bundle-owned, steps 5–6 are
     skipped for that resource. The app's data sources (e.g. the Vector Search
     index it queries) must already exist — the user provides them beforehand
     (typically via a separate pipeline DAB); quickstart only deploys the app
     against them, it does not build any data.
  5. MLflow experiment — if not already set from app resources (step 4) or bundle
     (step 4b), get username, seed MLFLOW_EXPERIMENT_ID from databricks.yml if not
     in .env, then create or reuse an experiment. Update .env and databricks.yml.
  6. Lakebase setup — skip if already resolved from app resources (step 4) or
     bundle (step 4b). Otherwise: if the template requires Lakebase (has LAKEBASE_*
     in databricks.yml) or CLI flags are provided, set up via CLI args or
     interactive selection. For non-memory templates, optionally offer Lakebase for
     chat UI history. Update databricks.yml resources and env vars.
...
def _load_yml(path: Path):
    ...
    # Keep long lines (e.g. ${resources...} references, descriptions) on one
    # line so round-trip edits don't reflow unrelated content.
    yaml.width = 4096
    with open(path) as f:
...
# ── Bundle (DAB) discovery ──────────────────────────────────────────────────
# The quickstart may run from a subdirectory (e.g. agent_app/) while the DAB
# root databricks.yml lives one or more levels up, with resources split across
# files referenced by `include:`. These helpers locate those files so we can
# read (and optionally rename) the experiment and Lakebase declared there,
# then let the bundle own resource creation.

def find_bundle_root() -> Path | None:
    """Walk up from CWD to find the DAB root: a databricks.yml with a `bundle:` key."""
    for base in [Path.cwd(), *list(Path.cwd().parents)[:6]]:
        dbx = base / "databricks.yml"
        if not dbx.exists():
            continue
        try:
            _, data = _load_yml(dbx)
        except Exception:
            continue
...
def bundle_yaml_files(root: Path) -> list[Path]:
    """Root databricks.yml plus every file matched by its `include:` globs."""
    files: list[Path] = []
    ...
    try:
        _, data = _load_yml(dbx)
        for pattern in (data or {}).get("include", []) or []:
...
def bundle_default_target(root: Path) -> str:
    """Return the bundle's default target name (or the first declared one)."""
    try:
        _, data = _load_yml(root / "databricks.yml")
    except Exception:
        return "dev"
    ...
    for name, cfg in targets.items():
        if isinstance(cfg, dict) and cfg.get("default"):
            return str(name)
    return str(next(iter(targets), "dev"))


def resolve_bundle_var(root: Path, value: str) -> tuple[str, Path | None, str | None]:
    """If `value` is exactly ${var.NAME}, resolve it to that variable's default.

    Returns (resolved_value, var_file, var_name). For a non-variable value,
    returns (value, None, None).
    """
    m = re.fullmatch(r"\$\{var\.([A-Za-z0-9_]+)\}", (value or "").strip())
    if not m:
    ...
    try:
        _, data = _load_yml(dbx)
        default = (data or {}).get("variables", {}).get(var_name, {}).get("default", "")
        return str(default), dbx, var_name
    except Exception:
        return value, None, None


def find_bundle_experiment() -> dict | None:
    """Locate an MLflow experiment declared in the bundle.

    Returns {"file", "key", "name", "root"} or None.
    """
    root = find_bundle_root()
    if not root:
        return None
    for f in bundle_yaml_files(root):
        try:
            _, data = _load_yml(f)
        except Exception:
            continue
        experiments = (data or {}).get("resources", {}).get("experiments", {}) or {}
        for key, val in experiments.items():
            if isinstance(val, dict) and val.get("name"):
                return {"file": f, "key": key, "name": str(val["name"]), "root": root}
    return None


def find_bundle_lakebase() -> dict | None:
    """Locate a Lakebase postgres project declared in the bundle.

    Returns {"file", "key", "project_id", "display_name", "root"} or None.
    """
    root = find_bundle_root()
    if not root:
        return None
    for f in bundle_yaml_files(root):
        try:
            ...
            continue
        projects = (data or {}).get("resources", {}).get("postgres_projects", {}) or {}
        for key, val in projects.items():
            if isinstance(val, dict):
                return {
                    ...
                    "display_name": str(val.get("display_name", "")),
                }
    return None


def resolve_experiment_name(name: str, username: str, target: str) -> str:
    """Substitute the DAB built-ins commonly used in experiment names."""
    return (
        name.replace("${workspace.current_user.userName}", username)
        .replace("${bundle.target}", target)
    )


def find_app_resource_file(root: Path) -> tuple[Path | None, str | None]:
    """Locate the yaml file that declares resources.apps and its first app key."""
    for f in bundle_yaml_files(root):
        try:
            _, data = _load_yml(f)
        except Exception:
...
def _prompt_rename(label: str, current: str) -> str | None:
    """Show a current value and let the user overwrite it. Returns a new value or None."""
    if not sys.stdin.isatty():
        return None
    answer = input(
        f"  {label}: press Enter to keep, or type a new value to overwrite the yaml: "
    ).strip()
...
def _run_bundle_deploy(root: Path, target: str, profile_name: str) -> bool:
    """Run `databricks bundle deploy` from the bundle root. Returns True on success."""
    print_step(f"Deploying bundle (target: {target})...")
    print("  This creates the experiment, Lakebase, and app declared in your DAB.")
    result = subprocess.run(
        ["databricks", "bundle", "deploy", "-t", target, "-p", profile_name],
        cwd=str(root),
    )
    if result.returncode != 0:
        print_error("Bundle deploy failed. Fix the errors above and re-run quickstart.")
        return False
    print_success("Bundle deployed")
    return True


def _bundle_summary(root: Path, target: str, profile_name: str) -> dict | None:
    """Return the parsed `databricks bundle summary -o json` for the deployed bundle."""
    result = subprocess.run(
        ["databricks", "bundle", "summary", "-t", target, "-p", profile_name, "-o", "json"],
        cwd=str(root),
        capture_output=True,
        text=True,
    )
...
def _bundle_lakebase_branch(root: Path) -> str:
    """Best-effort: read the Lakebase branch name from the bundle yamls (default 'production')."""
    for f in bundle_yaml_files(root):
        try:
            text = f.read_text()
        except Exception:
            continue
        m = re.search(r"/branches/([A-Za-z0-9_\-]+)", text)
        if m:
            return m.group(1)
    return "production"


def configure_bundle_resources(profile_name: str, username: str) -> dict | None:
    """DAB-aware setup: summarize the experiment/Lakebase declared in the bundle,
    let the user rename them (overwriting the yaml), then optionally deploy and
    read the real IDs back into .env.

    Returns:
        - None if the bundle declares no experiment/Lakebase (caller falls back
          to the standard create-your-own flow).
        - {"deployed": False} if resources were found but the user declined deploy.
        - {"deployed": True, "experiment_id": str|None, "endpoint": str|None} after
          a successful deploy + ID resolution.
    """
    exp = find_bundle_experiment()
    lake = find_bundle_lakebase()
    if not exp and not lake:
        return None

    root = (exp or lake)["root"]
    target = bundle_default_target(root)

    print_header("Bundle resources (Declarative Asset Bundle)")
    print("These resources are declared in your bundle. The bundle will create and")
    print("own them on deploy — quickstart just reads the resulting IDs into .env.\n")

    if exp:
        rel = exp["file"].relative_to(root)
        print(f"• MLflow experiment  ({rel} → experiments.{exp['key']})")
        print(f"    name: {exp['name']}")
    if lake:
        rel = lake["file"].relative_to(root)
        resolved_id, _, _ = resolve_bundle_var(root, lake["project_id"])
        print(f"• Lakebase project   ({rel} → postgres_projects.{lake['key']})")
        if lake["display_name"]:
            print(f"    display name: {lake['display_name']}")
        print(f"    project id:   {resolved_id or lake['project_id']}")

    # ── Allow renames (overwrite the yaml where the value lives) ──────────────
    exp_final_name = exp["name"] if exp else None
    if exp:
        new_name = _prompt_rename("Experiment name", exp["name"])
        if new_name:
            yaml, data = _load_yml(exp["file"])
            data["resources"]["experiments"][exp["key"]]["name"] = DoubleQuotedScalarString(new_name)
            _save_yml(yaml, data, exp["file"])
            print_success(f"Updated {exp['file'].relative_to(root)}: experiment name → {new_name}")
            exp_final_name = new_name

    if lake:
        resolved_id, var_file, var_name = resolve_bundle_var(root, lake["project_id"])
        shown = resolved_id or lake["project_id"]
        new_id = _prompt_rename("Lakebase project id", shown)
        if new_id:
            if var_name and var_file:
                yaml, data = _load_yml(var_file)
                data["variables"][var_name]["default"] = DoubleQuotedScalarString(new_id)
                _save_yml(yaml, data, var_file)
                print_success(
                    f"Updated {var_file.relative_to(root)}: variables.{var_name}.default → {new_id}"
                )
            else:
                yaml, data = _load_yml(lake["file"])
                data["resources"]["postgres_projects"][lake["key"]]["project_id"] = (
                    DoubleQuotedScalarString(new_id)
                )
                _save_yml(yaml, data, lake["file"])
                print_success(f"Updated {lake['file'].relative_to(root)}: project_id → {new_id}")

    # ── Deploy (with confirmation) ────────────────────────────────────────────
    if sys.stdin.isatty():
        answer = input(
            f"\nDeploy the bundle now to create these resources (target: {target})? [Y/n]: "
        ).strip().lower()
        if answer == "n":
            print_step("Skipping deploy.")
            print("  Run 'databricks bundle deploy' yourself, then re-run quickstart to")
            print("  populate .env with the created IDs.")
            return {"deployed": False}

    # The app's data sources (e.g. the Vector Search index) are provided by the
    # user beforehand (typically a separate pipeline DAB), so just deploy the app
    # against them — no data-building job to defer/run here.
    if not _run_bundle_deploy(root, target, profile_name):
        return {"deployed": False}

    resolved_exp_name = (
        resolve_experiment_name(exp_final_name, username, target) if exp_final_name else None
    )

    # ── Read the created IDs back into .env ───────────────────────────────────
    experiment_id: str | None = None
    endpoint: str | None = None
    summary = _bundle_summary(root, target, profile_name)

    if exp and summary:
        try:
            experiment_id = str(
                summary["resources"]["experiments"][exp["key"]].get("id", "")
            ) or None
        except (KeyError, TypeError):
            experiment_id = None
    if experiment_id:
        update_env_file("MLFLOW_EXPERIMENT_ID", experiment_id)
        print_success(f"MLFLOW_EXPERIMENT_ID → {experiment_id} (from bundle)")
    elif exp:
        print_error("Could not read experiment id from bundle summary; set MLFLOW_EXPERIMENT_ID manually.")

    if lake:
        resolved_id, _, _ = resolve_bundle_var(root, lake["project_id"])
        project = resolved_id or lake["project_id"]
        # Prefer the id the bundle actually created, if present.
        if summary:
            try:
                project = str(summary["resources"]["postgres_projects"][lake["key"]].get("id", project)) or project
            except (KeyError, TypeError):
                pass
        branch = _bundle_lakebase_branch(root)
        endpoint, host = _fetch_autoscaling_endpoint_info(profile_name, project, branch)
        if endpoint:
            update_env_file("LAKEBASE_AUTOSCALING_ENDPOINT", endpoint)
            print_success(f"LAKEBASE_AUTOSCALING_ENDPOINT → {endpoint} (from bundle)")
            if host:
                update_env_file("PGHOST", host)
        else:
            print_error(
                "Could not resolve the Lakebase endpoint automatically.\n"
                f"  Set LAKEBASE_AUTOSCALING_ENDPOINT for project '{project}' branch '{branch}' manually."
            )
...
Examples:
    ...
    try:
        ...
        # DAB-aware path: when NOT binding to an existing online app, let the
        # bundle own the experiment/Lakebase. Summarize the declared names, allow
        # renames (overwriting the yaml), deploy (with confirmation), then read
        # the created IDs into .env. Skipped when --app-name is used so the
        # existing "bring the binding from the online app" flow keeps priority.
        bundle_result = None
        if not app_name:
            bundle_result = configure_bundle_resources(profile_name, username)
        bundle_owned = bool(bundle_result and bundle_result.get("deployed"))
        ...
        if app_experiment_id:
            ...
            # The bundle created the experiment on deploy; .env is already set.
            experiment_id = bundle_result.get("experiment_id") or get_env_value("MLFLOW_EXPERIMENT_ID")
            experiment_name = bundle_result.get("experiment_name") or experiment_id or "(bundle-managed)"
            print_success("Experiment is bundle-managed (from deploy)")
        else:
        ...
        if bundle_owned:
            # Lakebase is bundle-managed; the endpoint was already written to .env
            # by the deploy step. Don't create another one or rewrite databricks.yml.
            print_step("Lakebase is bundle-managed (from deploy)")
            lakebase_config = None
        elif lakebase_config:
...
✓ Experiment ID: {experiment_id}"""
        ...
        if bundle_owned and bundle_result.get("endpoint"):
            summary += f"\n\n✓ Lakebase (bundle-managed): endpoint {bundle_result['endpoint']}"
        elif bundle_owned:
            summary += "\n\n✓ Lakebase is bundle-managed (set LAKEBASE_AUTOSCALING_ENDPOINT in .env if not resolved)"
        elif lakebase_config:
